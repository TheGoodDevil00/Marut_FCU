...
volatile uint32_t loop_dt_us = 0;
...
float PAngleRoll = 3.0f;
float PAnglePitch = 3.0f;
float IAngleRoll = 0.0f;
float IAnglePitch = 0.0f;
...
float PRateRoll = 0.52f;
float PRatePitch = 0.52f;
float PRateYaw = 2;
float IRateRoll = 3.65f;
float IRatePitch = 3.65f;
float IRateYaw = 12;
float DRateRoll = 0.02f;
float DRatePitch = 0.02f;
float DRateYaw = 0;
...
float vertical_velocity(void) {
	AccZInertial = -sin(kalman_pitch * (3.142 / 180))
			* (mpu_accel_read(0) - calibration_const_global_ax)
			+ cos(kalman_pitch * (3.142 / 180))
					* sin(kalman_roll * (3.142 / 180))
					* (mpu_accel_read(1) - calibration_const_global_ay)
			+ cos(kalman_pitch * (3.142 / 180))
					* cos(kalman_roll * (3.142 / 180))
					* (mpu_accel_read(2) - calibration_const_global_az);
...
*/
	...
	while (1) {
		...
		/* handle TIM10 wraparound */
		if (now < last_time)
		{
		    last_time = now;
		    //return;   // skip this loop safely
		}
		...
		if (dt >= 4000)
		{
		    loop_dt_us = dt;   // <-- THIS is what you plot
		    last_time = now;

		    /* rest of your control loop stays unchanged */
			...
			PAngleRoll = mapRCtoPID(display_channels[4]);
			PAnglePitch = mapRCtoPID(display_channels[4]);
